...
// 禁用页面加载期间滚动（尽早执行）。
// 如果 body 尚未解析，监听 DOMContentLoaded 后再设置。
function disableScrollDuringLoad() {
...
// 立即调用以尽早禁止滚动
disableScrollDuringLoad();

// Preloader Control
const pagePreloader = document.querySelector(".pre-loader");
window.addEventListener("load", () => {
    // 如果页面有预加载器，先触发一个短暂延迟以便配合 CSS 淡出（若存在），
    // 然后隐藏预加载器并恢复页面滚动。
    if (pagePreloader) {
        // 给页面添加一个可选的淡出类（若 CSS 未定义也不会有副作用）
        document.body.classList.add('preloader-fade-out');

        setTimeout(() => {
            pagePreloader.style.display = "none";
            document.body.classList.remove('preloader-fade-out');
            // 恢复滚动
            if (document.body) document.body.style.overflow = 'auto';
        }, 300); // 300ms 用于与潜在的 CSS 淡出动画配合
    } else {
        // 没有预加载器时直接恢复滚动
        if (document.body) document.body.style.overflow = 'auto';
    }
